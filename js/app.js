...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cloud Eval Elements
    const toggleCloudEval = document.getElementById('toggle-cloud-eval');
    const cloudEvalDisplay = document.getElementById('cloud-eval-display');
    const cloudEvalFeedback = document.getElementById('cloud-eval-feedback');
    let cloudEvalEnabled = false;
    let previousEval = { cp: undefined, mate: undefined, bestMoveUCI: null, fen: null };
    ...
    toggleCloudEval.addEventListener('change', (e) => {
        cloudEvalEnabled = e.target.checked;
        if (cloudEvalEnabled) {
            if (game.turn() === playerColor && !game.game_over()) {
                engine.findBestMove(game.fen(), 10);
            }
        } else {
            cloudEvalDisplay.textContent = 'OFF';
            cloudEvalDisplay.style.color = 'var(--text-secondary)';
            cloudEvalFeedback.style.display = 'none';
        }
    });

    // Handle Local Eval from Engine
    engine.onEval((data) => {
        if (!cloudEvalEnabled) return;

        let { cp, mate, pvs } = data;
        let bestMoveUCI = pvs ? pvs.split(' ')[0] : null;

        // Stockfish gives cp/mate relative to side to move. Normalize to White's view.
        const isWhiteToMove = game.turn() === 'w';
        if (cp !== undefined) cp = isWhiteToMove ? cp : -cp;
        if (mate !== undefined) mate = isWhiteToMove ? mate : -mate;
        ...
        if (mate !== undefined) {
            ...
            evalText = (mate > 0 ? "+" : "-") + evalText;
            color = mate > 0 ? "#4ade80" : "#ef4444";
        } else if (cp !== undefined) {
            const score = cp / 100;
            evalText = (score > 0 ? "+" : "") + score.toFixed(2);
            if (score > 1.0) color = "#4ade80";
            else if (score < -1.0) color = "#ef4444";
        }
        
        cloudEvalDisplay.textContent = evalText;
        cloudEvalDisplay.style.color = color;

        generateRealTimeFeedback({ cp, mate, moves: pvs }, game.fen());

        if (game.turn() === playerColor) {
            previousEval = { cp, mate, bestMoveUCI, fen: game.fen() };
        }
    });

    function generateRealTimeFeedback(newPv, fen) {
        if (game.turn() === playerColor) return; 
        if (!previousEval.fen) return;

        const newCp = newPv.cp;
        const newMate = newPv.mate;
        let scoreDrop = 0;
        
        if (previousEval.cp !== undefined && newCp !== undefined) {
            scoreDrop = playerColor === 'w' ? (previousEval.cp - newCp) : (newCp - previousEval.cp);
        }
        
        if (previousEval.mate !== undefined && newMate === undefined) {
            const hadMate = (playerColor === 'w' && previousEval.mate > 0) || (playerColor === 'b' && previousEval.mate < 0);
            if (hadMate) scoreDrop = 500; 
        } else if (previousEval.mate === undefined && newMate !== undefined) {
            const admitsMate = (playerColor === 'w' && newMate < 0) || (playerColor === 'b' && newMate > 0);
            if (admitsMate) scoreDrop = 500;
        }

        if (scoreDrop > 100) { 
            const pieces = {'p':'Pawn', 'n':'Knight', 'b':'Bishop', 'r':'Rook', 'q':'Queen', 'k':'King'};
            
            const dummy = new Chess(previousEval.fen);
            let bestMoveMsg = "finding a better tactical sequence.";
            
            if (previousEval.bestMoveUCI) {
                const fromSq = previousEval.bestMoveUCI.substring(0, 2);
                const toSq = previousEval.bestMoveUCI.substring(2, 4);
                const prom = previousEval.bestMoveUCI.length === 5 ? previousEval.bestMoveUCI[4] : undefined;
                const moveObj = dummy.move({from: fromSq, to: toSq, promotion: prom || 'q'});
                
                if (moveObj) {
                    const pieceName = pieces[moveObj.piece];
                    
                    if (moveObj.flags.includes('c') || moveObj.flags.includes('e')) {
                        const capturedPiece = moveObj.captured ? pieces[moveObj.captured] : "piece";
                        bestMoveMsg = `capturing the ${capturedPiece} on ${moveObj.to} with your ${pieceName}.`;
                    } else if (moveObj.san.includes('+')) {
                        bestMoveMsg = `playing ${pieceName} to ${moveObj.to} to force a check.`;
                    } else {
                        bestMoveMsg = `playing ${pieceName} to ${moveObj.to}.`;
                    }
                }
            }
            
            let punishmentMsg = "";
            const newBestMoveUCI = newPv.moves ? newPv.moves.split(' ')[0] : null;
            
            if (newMate !== undefined) {
                const isOpponentMate = (playerColor === 'w' && newMate < 0) || (playerColor === 'b' && newMate > 0);
                if (isOpponentMate) {
                    punishmentMsg = Math.abs(newMate) === 1 ? " This allows the opponent to deliver checkmate!" : ` This allows a forced mate in ${Math.abs(newMate)}!`;
                }
            } else if (newBestMoveUCI) {
                const currentDummy = new Chess(fen);
                const fromSq2 = newBestMoveUCI.substring(0, 2);
                const toSq2 = newBestMoveUCI.substring(2, 4);
                const prom2 = newBestMoveUCI.length === 5 ? newBestMoveUCI[4] : undefined;
                const oppMoveObj = currentDummy.move({from: fromSq2, to: toSq2, promotion: prom2 || 'q'});
                
                if (oppMoveObj && (oppMoveObj.flags.includes('c') || oppMoveObj.flags.includes('e'))) {
                    const capturedPiece = oppMoveObj.captured ? pieces[oppMoveObj.captured] : "piece";
                    punishmentMsg = ` This allows the opponent to directly capture your ${capturedPiece}!`;
                }
            }
            
            let label = scoreDrop > 250 ? "Blunder!" : (scoreDrop > 150 ? "Mistake!" : "Inaccuracy.");
            let color = scoreDrop > 250 ? "#ef4444" : "#f59e0b"; 
            
            cloudEvalFeedback.innerHTML = `<span style="color:${color}; font-weight:bold;">${label}</span> You missed an opportunity by not ${bestMoveMsg}${punishmentMsg}`;
            cloudEvalFeedback.style.display = 'block';
            cloudEvalFeedback.style.borderLeftColor = color;
        } else {
            cloudEvalFeedback.style.display = 'none';
        }
    }
    ...
    function startNormalGame(color) {
        isTacticsMode = false;
        previousEval = { cp: undefined, mate: undefined, bestMoveUCI: null, fen: null };
        cloudEvalFeedback.style.display = 'none';
    ...
    btnTactics.addEventListener('click', () => {
        ...
        let fen = typeof randomPuzzle === 'string' ? randomPuzzle : randomPuzzle.fen;
        currentPuzzleDescription = typeof randomPuzzle === 'string' ? "Tactics Puzzle" : randomPuzzle.description;
    ...
    engine.onMove((bestMove) => {
        if (game.turn() === playerColor) {
            // Evaluated the human's position. Do not auto-play a move for the human!
            return;
        }
        // e.g. e2e4
    ...
    function updateStatus() {
        ...
        if (isTacticsMode && currentPuzzleDescription && !game.game_over()) {
            status += ` — ${currentPuzzleDescription}`;
        }
