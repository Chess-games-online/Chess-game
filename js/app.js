...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchCloudEval(fen) {
        ...
        try {
            cloudEvalDisplay.textContent = "evaluating...";
            cloudEvalDisplay.style.color = "var(--text-secondary)";
            ...
            if (res.status === 200) {
                ...
                if (data.pvs && data.pvs.length > 0) {
                    ...
                    if (pv.mate !== undefined) {
                        evalText = "M" + Math.abs(pv.mate);
                        evalText = (pv.mate > 0 ? "+" : "-") + evalText;
                        color = pv.mate > 0 ? "#4ade80" : "#ef4444";
                    ...
                    generateRealTimeFeedback(pv, fen);

                    previousEval = {
                        cp: pv.cp,
                        mate: pv.mate,
                        bestMoveUCI: pv.moves ? pv.moves.split(' ')[0] : null,
                        fen: fen
                    };
                } else {
                    cloudEvalDisplay.textContent = "No PV Data";
                }
            } else {
                cloudEvalDisplay.textContent = "novel position";
                cloudEvalDisplay.style.color = "var(--text-secondary)";
            }
        } catch (e) {
            cloudEvalDisplay.textContent = "error";
        }
    }
